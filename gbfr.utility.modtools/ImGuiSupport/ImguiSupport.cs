...
public unsafe class ImguiSupport
{
    ...
    private unsafe static void ConfigureImgui(string modFolder)
    {
        var io = ImGui.GetIO();

        // I GIVE UP. MergeMode does NOT work.
        // Set it on the roboto one, it crashes. It will always crash if a config is provided with MergeMode = true to the first font.
        // If you don't set it and allow roboto to initialize, second font gets 'loaded' (at least imgui registers the glyphs as seen with the style editor), but the atlas doesn't build. WTF?
        // So whatever, fuck this shit. Only load noto.

        var intern = new ImFontConfig.__Internal();
        var conf = new ImFontConfig(&intern);
        conf.MergeMode = true;
        conf.FontData = 0;

        string robotoFontPath = Path.Combine(modFolder, "Fonts", "Roboto", "Roboto-Medium.ttf");
        //ImGui.ImFontAtlasAddFontFromFileTTF(io.Fonts, robotoFontPath, 15.0f, null, ref ImGui.ImFontAtlasGetGlyphRangesDefault(io.Fonts)[0]);

        string netoSansJpFontPath = Path.Combine(modFolder, "Fonts", "Noto", "NotoSansJP-Medium.ttf");
        //ImGui.ImFontAtlasAddFontFromFileTTF(io.Fonts, netoSansJpFontPath, 17.0f, conf, ref ImGui.ImFontAtlasGetGlyphRangesJapanese(io.Fonts)[0]);
        ImGui.ImFontAtlasAddFontFromFileTTF(io.Fonts, netoSansJpFontPath, 17.0f, null, ref ImGui.ImFontAtlasGetGlyphRangesJapanese(io.Fonts)[0]);
    ...
    public void Render()
    {
        if (!_noticeShown)
        {
            OverlayLogger.Instance.AddMessage("Mod Tools loaded. Press the INSERT key to show the main menu.");
            _noticeShown = true;
        }
